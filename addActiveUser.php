...
$sqlCheckActivate="select count(id) as count from activate where userid=? and chatid=?";
...
$sqlNewActivate="insert into activate (chatid,userid) values(?,?)";
$stmt = $mysqli->prepare($sqlNewActivate);
$stmt->bind_param('ii', $id,$userid);
/* выполнение подготовленного запроса */
$stmt->execute();
$stmt->close();
